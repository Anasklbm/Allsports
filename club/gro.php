...
if(isset($_POST['b12']))
{
    ...
    $lo=$_POST['t2'];
      ...
      $_SESSION['dt1']=$t4;
						...
						<div class="blog-lagre-view theme-padding-bottom">
                                                     ...
                                                     <?php
                                                        ...
                                                        $sel1=mysqli_query($dbcon,"SELECT id, SQRT( POW(69.1 * (la - $la), 2) + POW(69.1 * ($lo - lo) * COS(la / 57.3), 2)) AS distance FROM gro_data where st='1' and typ='$t3' HAVING distance < 60 ORDER BY distance");
                                                        while($r1=mysqli_fetch_row($sel1))
                                                            {
                                                        
                                                        $sel_gal=mysqli_query($dbcon,"select * from gro_data where id='$r1[0]'");
							...
							<div class="large-post theme-padding-bottom">
								...
								<div class="large-post-img">
                                                                    <img style="width: 100%;height: 500px" src="../img2/<?php echo $r_gal[9] ?>" alt="">
									<span class="blog-title-batch">Sports Ground</span>
								</div>
								<!-- Post Img -->
								...
								<div class="large-post-detail">
									<div class="author-header">
										<div class="aurhor-img-name pull-left">
                                                                                    <img style="width: 50px;height: 50px" src="../pic/sport-equipment-concept_1284-13034.jpg" alt="">
                                                                                 
                                                                                   <?php
            
                                                        $sel_gal1=mysqli_query($dbcon,"select * from g_typ where gid='$r_gal[7]' ");
                                                         if(mysqli_num_rows($sel_gal1)>0)
                                                        {
                                                        while($r_gal1=mysqli_fetch_row($sel_gal1))
                                                            {
                                                        ?> 
                                                                                    <a class="btn gary-btn" target="_blank" href="gro_dt.php?mid=<?php echo $r_gal[7] ?>#<?php echo $r_gal1[0] ?>"><i>+</i><?php echo $r_gal1[1] ?></a>
     ...
     ?>
         ...
         <a class="btn btn-danger" href="#"><i>-</i>No Ground Found</a>                                                                           
   <?php
 }
                                                            ?>
                                                                                  </div>
									...
									<p><?php echo $r_gal[3]?></p>
                                                                        <a class="btn btn-success "target="_blank" href="gro_dt.php?mid=<?php echo $r_gal[7] ?>"><i>+</i>Book Now</a>
								</div>
								<!-- Detail -->
...
?>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDge58kDZRSDquz16IhEFQZMzHjMpFWwlw&libraries=places&callback=initAutocomplete"
         async defer></script>
                ...
                <input type="text"required=""class="form-control" name="t1"id="lat"placeholder="Double Click on the map" value="">
            ...
            <div class="form-group"> 	 
                <label for="firstname"><span class="req">* </span>Type </label>
...
<div class="form-group"> 	 
                <label for="firstname"><span class="req">* </span> date: </label>
