...
if(isset($_POST['sub1']))
{
    ...
    $fname=$count."".substr($up,strrpos($up,"."));
...
$usrc=mysqli_query($dbcon,"select * from user_login where uid='$t5'");
   ...
   $up=$_FILES['f2']['name'];
    $count=rand('1000000','9999999');
    $fname=$count."".substr($up,strrpos($up,"."));
if(move_uploaded_file($_FILES['f2']['tmp_name'],getcwd()."\\state/img1\\$fname"))
{ 
    $ins=mysqli_query($dbcon,"insert into add_club1 values('','$t1','$t2','$t3','$t4','$t5','$t6','$fname','0','0','0','0','0','0','$ta','0')");
    if($ins>0)
    {
...
<div class="wrap push">
	...
	<main class="main-content">
		...
		<div class="error-holder theme-padding">
            ...
            <div class="form-group">
                <label for="lastname"><span class="req">* </span> Sports Department: </label> 
                <select name="ta" class="form-control">
                    
                <option>--select Department--</option>
                <?php
                
                                        $sel_state=mysqli_query($dbcon,"select * from ad_st ");
                                        while($r_state=mysqli_fetch_row($sel_state))
                                        {
                                            ?>
                                        <option value="<?php echo $r_state[6] ?>"><?php echo $r_state[2] ?></option>
                                       <?php
                                        }
                                        ?>
                </select>
...
<div class="form-group">
            ...
            <div class="form-group">
                ...
                <label for="lastname"><span class="req">* </span> Photo: </label> 
                    <input class="form-control" type="file" name="f2"  placeholder="hyphen or single quote OK" required />  
                        <div id="errLast"></div>
            </div>
